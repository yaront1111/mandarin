  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const user = await User.findById(req.params.id).select(
        'nickname details photos isOnline lastActive createdAt'
      );
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (nickname && nickname.trim().length < 3) {
        return res
          .status(400)
          .json({ success: false, error: 'Nickname must be at least 3 characters' });
      }
      if (details && details.age && (details.age < 18 || details.age > 120)) {
        return res
          .status(400)
          .json({ success: false, error: 'Age must be between 18 and 120' });
      }
      ...
      if (details) {
        ...
        if (details.age !== undefined) {
          updateData.details.age = parseInt(details.age, 10);
        }
        ...
        if (details.bio !== undefined) {
          if (details.bio.length > 500) {
            return res
              .status(400)
              .json({ success: false, error: 'Bio cannot exceed 500 characters' });
          }
        ...
        if (details.interests !== undefined) {
          if (typeof details.interests === 'string') {
            ...
            if (details.interests.length > 10) {
              return res
                .status(400)
                .json({ success: false, error: 'Cannot have more than 10 interests' });
            }
            updateData.details.interests = details.interests;
          }
        }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(photoId) || !mongoose.Types.ObjectId.isValid(userId)) {
      return res
        .status(400)
        .json({ success: false, error: 'Invalid photo ID or user ID format' });
    }
    ...
    if (
      ...
      new Date(permission.createdAt).getTime() < Date.now() - 1000
    ) {
      logger.warn(`Photo access request failed: Request already exists`);
      return res
        .status(400)
        .json({ success: false, error: 'Permission request already exists' });
    }
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!['approved', 'rejected'].includes(status)) {
      ...
      return res
        .status(400)
        .json({ success: false, error: 'Status must be either "approved" or "rejected"' });
    }
    ...
    if (!permission) {
      ...
      return res
        .status(404)
        .json({ success: false, error: 'Permission request not found' });
    }
    ...
    if (!owner) {
      ...
      return res
        .status(401)
        .json({ success: false, error: 'Not authorized to update this permission' });
    }
