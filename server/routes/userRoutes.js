...
router.get('/:id', protect, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data: { user, messages, messagesPagination: { total: totalMessages, page, pages: Math.ceil(totalMessages / limit) } }
    });
  } catch (err) {
...
router.put('/profile', protect, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (details) {
      ...
      if (details.age !== undefined) updateData.details.age = parseInt(details.age);
...
router.post('/photos', protect, upload.single('photo'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const photo = { url: `/uploads/${req.file.filename}`, isPrivate, metadata: photoMetadata };
...
router.put('/photos/:id/profile', protect, asyncHandler(async (req, res) => {
  ...
  const photo = user.photos.splice(user.photos.findIndex(p => p._id.toString() === photoId), 1)[0];
...
router.delete('/photos/:id', protect, asyncHandler(async (req, res) => {
  ...
  if (user.photos[0]._id.toString() === photoId) {
    return res.status(400).json({ success: false, error: 'Cannot delete your profile photo. Set another photo as profile first.' });
  }
  const photoIndex = user.photos.findIndex(p => p._id.toString() === photoId);
  ...
  // Optionally, delete related photo permissions if applicable.
...
router.post('/photos/:id/request', protect, asyncHandler(async (req, res) => {
  ...
  permission = await PhotoPermission.create({ photo: photoId, requestedBy: req.user._id });
