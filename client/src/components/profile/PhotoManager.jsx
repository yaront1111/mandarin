...
const PhotoManager = ({ photos, onPhotosUpdate }) => {
  ...
  // Cleanup local object URLs on unmount or when filePreviewUrls changes.
  useEffect(() => {
    return () => {
      filePreviewUrls.forEach((url) => revokeLocalImageUrl(url));
    };
  }, [filePreviewUrls]);
  ...
  const handleFileSelect = async (e) => {
    ...
    // Create local preview URLs (if you wanted to show them before uploading, you could store them)
    const newPreviews = files.map((file) => createLocalImageUrl(file));
      ...
      const uploadPromises = files.map(async (file, index) => {
        ...
        const updateProgress = () => {
          setUploadProgress((prevProgress) =>
            Math.min(
              prevProgress + 100 / files.length / 10,
              (index + 1) * (100 / files.length)
            )
          );
        };
        ...
        try {
          const newPhoto = await photoService.uploadPhoto(file, isPrivate, {
            caption: '',
          });
  ...
  // Handle manual reordering using arrow buttons
  ...
  const togglePhotoPrivacy = async (photoId, currentStatus) => {
    try {
      ...
      onPhotosUpdate(
        photos.map((photo) =>
          photo.id === photoId ? { ...photo, isPrivate: !currentStatus } : photo
        )
      );
    } catch (error) {
  ...
  const savePhotoDetails = async () => {
    try {
      ...
      onPhotosUpdate(
        photos.map((photo) =>
          photo.id === selectedPhotoId ? { ...photo, caption: photoCaption } : photo
        )
      );
      closePhotoDetails();
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          ...
          <label className="px-4 py-2 bg-brand-pink text-white rounded-md hover:bg-opacity-90 cursor-pointer">
            Add Photos
            <input
              ref={fileInputRef}
              type="file"
      ...
      {photos.length === 0 ? (
        <div className="bg-bg-card p-10 rounded-lg border-2 border-dashed border-gray-700 text-center">
          ...
          <p className="text-text-secondary mb-4">
            No photos yet. Add photos to improve your profile!
          </p>
          <label className="px-4 py-2 bg-brand-pink text-white rounded-md hover:bg-opacity-90 cursor-pointer inline-block">
            Upload Photos
            <input
              type="file"
              multiple
              ...
              <div className="absolute left-2 top-1/2 transform -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  className={`w-8 h-8 rounded-full bg-black bg-opacity-50 flex items-center justify-center text-white ${
                    index === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-opacity-70'
                  }`}
                >
                  <svg
                    width="16"
              ...
              <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                >
                  ...
                  >
                    {photo.isPrivate ? (
                      <path
                        d="M8 11V7C8 5.93913 8.42143 4.92172 9.17157 4.17157C9.92172 3.42143 10.9391 3 12 3C13.0609 3 14.0783 3.42143 14.8284 4.17157C15.5786 4.92172 16 5.93913 16 7V11M5 11H19C20.1046 11 21 11.8954 21 13V20C21 21.1046 20.1046 22 19 22H5C3.89543 22 3 21.1046 3 20V13C3 11.8954 3.89543 11 5 11Z"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                      ...
                      <path
                        d="M15 11V7C15 5.93913 14.5786 4.92172 13.8284 4.17157C13.0783 3.42143 12.0609 3 11 3C9.93913 3 8.92172 3.42143 8.17157 4.17157C7.42143 4.92172 7 5.93913 7 7M19 11H5C3.89543 11 3 11.8954 3 13V20C3 21.1046 3.89543 22 5 22H19C20.1046 22 21 21.1046 21 20V13C21 11.8954 20.1046 11 19 11Z"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
            ...
            <div className="flex-grow overflow-auto p-4">
              ...
              <div className="mb-4">
                <label
                  ...
                  className="block text-text-primary mb-1 font-medium"
                >
                  Caption
                </label>
                <input
              ...
              <div className="flex items-center">
                <label className="inline-flex items-center">
                  <input
                    ...
                    onChange={() =>
                      togglePhotoPrivacy(selectedPhoto?.id, selectedPhoto?.isPrivate)
                    }
                    className="mr-2 h-4 w-4 text-brand-pink rounded border-gray-300 focus:ring-brand-pink"
                  />
