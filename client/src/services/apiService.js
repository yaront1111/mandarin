  ...
  async processQueue() {
    ...
    for (let i = 0; i < this.queue.length; i++) {
      ...
      try {
        ...
        request.retries++;
        this.logger.warn(`Failed to process queued request (attempt ${request.retries}): ${request.method} ${request.url}`);

        if (request.retries >= this.maxRetries) {
          completedIndices.push(i);
          this.logger.error(`Giving up on queued request after ${this.maxRetries} attempts: ${request.method} ${request.url}`);
        }
      }
    }
    ...
    this.saveToStorage();
    this.isProcessing = false;
    this.logger.info(`Queue processing complete. ${completedIndices.length} requests processed, ${this.queue.length} remaining`);
  }
}
...
class ApiService {
  constructor() {
    ...
    this.cache = new ResponseCache(
      parseInt(process.env.REACT_APP_CACHE_SIZE || '100'),
      parseInt(process.env.REACT_APP_CACHE_TTL || '60000')
    );
  ...
  _handleNetworkStatusChange(isOnline) {
    ...
    if (typeof window !== 'undefined' && window.dispatchEvent) {
      window.dispatchEvent(new CustomEvent('apiConnectionStatusChange', {
        detail: { isOnline }
      }));
    }
  }
...
_loadAuthToken() {
  ...
  _handleResponse(response) {
    ...
    this.metrics.avgResponseTime = (this.metrics.avgResponseTime * (this.metrics.successfulRequests - 1) + requestTime) / this.metrics.successfulRequests;
    ...
    this.logger.debug(`Response success: ${response.config.method.toUpperCase()} ${response.config.url} (${requestTime}ms)`);
  ...
  async _processHttpError(error) {
    ...
    this.logger.error(`Response error: ${originalRequest.method.toUpperCase()} ${originalRequest.url}`,
      `Status: ${status}`, error.response.data);
