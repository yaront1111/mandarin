...
class ApiService {
  ...
  _handleRequest(config) {
    ...
    this.logger.debug(`Request: ${config.method.toUpperCase()} ${config.url}`, config.params ? `Params: ${JSON.stringify(config.params)}` : "");
    return config;
  }
  ...
  _handleResponseError(error) {
    ...
    if (!error.response) {
      ...
      const errorMsg = isOffline ? "You are currently offline. Please check your connection." : "Network error. Please try again.";
