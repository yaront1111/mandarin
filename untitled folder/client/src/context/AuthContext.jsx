...
export const AuthProvider = ({ children }) => {
  // Enhanced user validation and cleanup
  const validateAndSetUser = (userData) => {
    ...
    if (!cleanUserData._id || !/^[0-9a-fA-F]{24}$/.test(cleanUserData._id)) {
      log.error(`Invalid user ID format: ${cleanUserData._id}`);
      log.error(`ID type: ${typeof cleanUserData._id}, Length: ${cleanUserData._id?.length || 0}`);
      
      // If we're in development, still set the user but warn
      if (process.env.NODE_ENV === 'development') {
        console.warn('Setting user with invalid ID format for development purposes');
        console.warn('User data:', cleanUserData);
      ...
      log.debug(`Setting valid user: ${cleanUserData._id} (${typeof cleanUserData._id})`);
      setUser(cleanUserData);
    }
  };
